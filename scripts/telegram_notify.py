...
from disparity import Snapshot, zone_emoji, OVERHEAT, COOLDOWN
...
def build_message(snap: Snapshot) -> str:
    ...
    lines = [
        f"📊 *코스피 50일 이격도* — {snap.type_label}",
        f"🗓 {snap.date} {snap.time} KST",
        "",
        f"{emoji} *{snap.disparity:.1f}%*  ·  {snap.zone_label}{disp_delta}",
        "",
        f"• 코스피: *{snap.kospi:,.2f}*  {change_line}",
    ...
    lines += [
        "",
        f"기준: 이그전(이은택의 그림전략) · 과열 ≥{OVERHEAT:.0f}% / 해소 ≤{COOLDOWN:.0f}%",
        f"{snap.note}",
    ]
    return "\n".join(lines)
