...
(function () {
  ...
  // 통합 신호: MDD 구간 + 이격도 → (class, emoji, 결론, 설명)  ※ MDD 페이지와 동일 로직
  ...
  function renderSignals(latest) {
    ...
    $("signalRows").innerHTML = order.map((key) => {
      ...
      if (!idx) return "";
      const [cls, emoji, verdict, desc] = combinedSignal(idx.dd52, idx.disparity, key);
      return `<div class="sig ${cls}">
        <span class="sig-emoji">${emoji}</span>
        <div class="sig-main">
          <div class="sig-idx">${idx.name} · 이격도 ${fmt(idx.disparity, 1)} · MDD ${signed(idx.dd52, 1)}%</div>
  ...
  function renderDualCards(latest) {
    ...
    $("dualCards").innerHTML = order.map((key) => {
      ...
      const [mzk, mzl] = mddZone(idx.dd52, key);
      ...
      return `<div class="idx-card">
        ...
        <div class="idx-stats">
          ...
          <div class="row"><span class="k">고점대비 낙폭(MDD)</span><span class="v z-${mzk}">${signed(idx.dd52, 1)}%<span class="sub">${mzl}</span></span></div>
        </div>
      </div>`;
    }).join("");
  }
  ...
  function emptyState() {
    $("signalRows").innerHTML = `<div class="sig neutral"><span class="sig-emoji">⏳</span><div class="sig-main"><div class="sig-verdict">데이터 불러오는 중…</div><div class="sig-desc">MDD 페이지의 공용 데이터를 읽지 못했습니다. 잠시 후 새로고침 해주세요.</div></div></div>`;
  }
